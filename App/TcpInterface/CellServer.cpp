...
CellServer::CellServer(SOCKET sock)
{
    ...
    _oldTime = CTime::getNowInMilliSec();
}
...
//    m_pthread.detach();
    ...
    m_pthread.Start(nullptr,[this](CBaseThread* pThread){OnRun(pThread);},[this](CBaseThread* pThread){ClearClients();});
    m_TaskServer.StartTask();
}


//只用来读取客户端发来的数据
bool CellServer::OnRun(CBaseThread* pThread)
{
    fd_set _fdead_back;
...
//        _clients_change = WriteData(fdWrite);
        ...
        //check heart
        auto nowTime = CTime::getNowInMilliSec();
...
void CellServer::setEventOj(CBaseEvent* event)
{
    m_pNetEvent = event;
}
